			...
			<div class="flex-1 bg-gradient-to-r">
				<div class="flex-1 m-1 p-1">
					<v2-table
						...
						@page-change="getAccount_statement"
						border
					...
					>
						...
						<v2-table-column label="invoice_nuber" prop="invoice_nuber">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.invoice_nuber | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_to_user" prop="payment_to_user">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_to_user | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_date" prop="payment_date">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_date | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_type" prop="payment_type">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_type | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_amount" prop="payment_amount">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_amount | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_currency" prop="payment_currency">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_currency | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="currency_exchange_rate" prop="currency_exchange_rate">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.currency_exchange_rate | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="payment_status" prop="payment_status">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.payment_status | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						...
						<v2-table-column label="credit_debit" prop="credit_debit">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.credit_debit | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="debit" prop="debit">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.debit | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="credit" prop="credit">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.credit | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						
						<v2-table-column label="balance" prop="balance">
							<template slot-scope="scope">
								<div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
									{{scope.row.balance | truncate(25)}}
								</div>
							</template>
						</v2-table-column>
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									@click="$nuxt.$emit('evtUpsertAccount_statement',scope.row)"
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									v-if="!scope.row.mark_as_delete"
									@click="$nuxt.$emit('evtDeleteAccount_statement',scope.row)"
									class="inline-flex space-x-2 text-gray-400 hover:text-gray-500"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
...
<script>
	    /**
	     * Account_statementDataTable.vue
	     *
	     * @description :: Datatable / grid to list [Account_statement] records
	    ...
	    methods: {
	        async getAccount_statement(page) {
	            try {
	                this.currentPage = page;
	                this.loading = false;
	                let offset = page == 1 ? 0 : (page - 1) * pageSize;
	                offset = isNaN(offset) ? 0 : offset;
	                let api = this.$config.apiURL  + "Account_statement?sort=id%20DESC&limit=" + pageSize + "&skip=" + offset;
	                ...
	                return this.Account_statement = data;
	            } catch (error) {
	                console.log(error)
	            }
	    ...
	    created() {
	        ...
	        //1) Bind Account_statement Datatable when the view is mounted
	        this.getAccount_statement(this.currentPage);

	        //2) After successful Insert or update or Delete of the record, rebind the datatable
	        this.$nuxt.$on('evtBindAccount_statementDatatable', (data) => {
	            if (data !== undefined) {
	        ...
	        //3) Refresh table
	        this.$nuxt.$on('evtRefreshAccount_statementDatatable', (data) => {
	            this.getAccount_statement(this.currentPage);
	        });
	        //4) Search
	        this.$nuxt.$on('evtSearchAccount_statement', (data) => {
	            //alert(JSON.stringify(data))
	            this.searchCriteria = data;
	            this.getAccount_statement(1);
	        });
	    },
	};
</script>
